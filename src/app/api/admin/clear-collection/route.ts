...
export async function POST(req: Request) {
  try {
    await requireAdmin();
    const body = (await req.json().catch(() => ({}))) as { email?: string; confirm?: string };
    ...
    const { error } = await admin.from("collection_items").delete().eq("user_id", member.id);
    if (error) throw error;
    console.warn(`admin cleared collection: ${member.email} (${count ?? 0} rows)`);
    return NextResponse.json({ ok: true, member: member.email, removed: count ?? 0 });
  } catch (err) {
